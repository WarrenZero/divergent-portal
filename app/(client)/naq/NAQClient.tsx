...
interface Props {
  firstName: string;
  hasPriorNAQ?: boolean;
  priorScores?: NAQDomainScore[] | null;
}

// ─── Client-facing domain name simplification ─────────────────

const CLIENT_DOMAIN_NAMES: Record<string, string> = {
  'GI / Digestion':        'Digestion',
  'Immune / Lymph':        'Immune System',
  'Liver / Detox':         'Liver Health',
  'Fatty Acids':           'Essential Fats',
  'Minerals / Hydration':  'Minerals & Hydration',
};

function clientDomainName(name: string): string {
  return CLIENT_DOMAIN_NAMES[name] ?? name;
}
...
export default function NAQClient({ firstName, hasPriorNAQ = false, priorScores = null }: Props) {
  ...
  const [pillHintVisible, setPillHintVisible] = useState(true);
    ...
    return (
      <div className={styles.page}>
        <div className={styles.main} style={{ maxWidth: 560, margin: '0 auto', paddingTop: 48 }}>
          ...
          <div style={{ background: 'var(--pine-100)', border: '1px solid var(--pine-200)', borderRadius: 'var(--radius)', padding: '12px 16px', marginBottom: hasPriorNAQ ? 16 : 32, fontFamily: "'Lora', Georgia, serif", fontStyle: 'italic', fontSize: '13px', color: 'var(--pine-600)', lineHeight: 1.5 }}>
          ...
          {hasPriorNAQ && (
            <div style={{
              ...
              ✦ You&rsquo;ve completed this assessment before. Retaking it will show you how your health has changed — Warren uses this comparison to adjust your protocol.
            </div>
          )}
      ...
      <div className={styles.completionPage}>
          ...
          {domainScores.map((ds) => (
            <div key={ds.domainId} className={styles.domainScoreCard}>
              <div className={styles.domainScoreHeader}>
                ...
                <span className={styles.domainScoreName}>{clientDomainName(ds.name)}</span>
              </div>
        ...
        <button
          ...
          onClick={() => {
            const lines = [
              ...
              '',
              'Domain Burden:',
              ...domainScores.map((d) => `  ${clientDomainName(d.name)}: ${Math.round(d.burden)}% burden — ${burdenLabel(d.burden)}`),
              '',
        ...
        {priorScores && (
          <div className={styles.comparisonSection}>
            <div className={styles.comparisonTitle}>✦ Your Progress Since Last Assessment</div>
            <div className={styles.comparisonGrid}>
              {domainScores.map((ds) => {
                const prior = priorScores!.find((p) => p.domainId === ds.domainId);
                if (!prior) return null;
                const diff = prior.burden - ds.burden; // positive = improved (less burden)
                const arrow = diff > 3 ? '↓' : diff < -3 ? '↑' : '→';
                const arrowColor = diff > 3 ? 'var(--pine-500)' : diff < -3 ? '#C45C40' : 'var(--copper-400)';
                const changeLabel = diff > 3 ? 'Improved' : diff < -3 ? 'Higher burden' : 'No change';
                return (
                  <div key={ds.domainId} className={styles.comparisonRow}>
                    <span className={styles.comparisonDomain}>{clientDomainName(ds.name)}</span>
                    <span className={styles.comparisonValues}>
                      {prior.burden}% → {ds.burden}%
                    </span>
                    <span className={styles.comparisonArrow} style={{ color: arrowColor }}>
                      {arrow} {changeLabel}
                    </span>
                  </div>
                );
        ...
        <div className={styles.completionActions}>
          <p className={styles.completionNotify}>
            Warren has been notified that your assessment is complete.
          </p>
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.progressHeader}>
        ...
        <div
          className={styles.domainPills}
          ref={pillsRef}
          onScroll={() => setPillHintVisible(false)}
        >
          {NAQ_DOMAINS.map((d, i) => {
            const cls =
              i < domainIdx
        ...
        {pillHintVisible && (
          <div className={styles.pillScrollHint}>← scroll to see all domains →</div>
        )}
      </div>
